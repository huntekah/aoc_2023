...
def solve_puzzle(puzzle: Puzzle) -> int:
    ...
    c = 3
    distance = solve_with_djikstra(field, start, stop, c)
    return distance


def solve_with_djikstra(field: Field, start: Location, stop: Location, c: int) -> int:
    ...
    while len(active_nodes) > 0:
        ...
        for direction in get_directions(c_last_direction, c_dild, c):
...
@cache
def get_directions(
    last_direction: Directions, distance_in_last_direction: int, c: int
) -> set[Directions]:
    ...
    if distance_in_last_direction < c:
