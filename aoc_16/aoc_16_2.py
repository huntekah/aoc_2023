...
def solve_puzzle(puzzle) -> int:
    # find all possible starting locations and get highest score
    rays_to_check = []
    for i in range(len(puzzle)):
        rays_to_check.append(((i, -1), RIGHT))
        rays_to_check.append(((i, len(puzzle)), LEFT))
    for i in range(len(puzzle[0])):
        rays_to_check.append(((-1, i), DOWN))
        rays_to_check.append(((len(puzzle), i), UP))
    return max([try_starting_from(puzzle, *ray) for ray in rays_to_check])


def try_starting_from(puzzle, location: Location, direction: Directions) -> int:
    field = Field(puzzle)
